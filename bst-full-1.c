...
{
	if(ptr == NULL)	//ptr널이면 널 리턴
		return NULL;

	if(ptr->key < key)	//찾는 키가 더 크면 오른쪽 또 호출
		ptr = searchRecursive(ptr->right, key);
	else if(ptr->key > key)
		ptr = searchRecursive(ptr->left, key);

	return ptr;//작지도크지도 않을때(같을때)리턴

}

Node* searchIterative(Node* head, int key)
{
	Node* ptr = head->left;	//ptr에 첫노드넣기

	while(ptr != NULL)		//끝까지
	{
		if(ptr->key == key)	//키가 같은 노드 리턴
			return ptr;

		if(ptr->key < key)	//찾는 키가 더 크면 ptr 에 오른쪽 넣고 아니면 왼쪽을 넣음
			ptr = ptr->right;	
		else
			ptr = ptr->left;
	}
